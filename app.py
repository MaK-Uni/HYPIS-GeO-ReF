...
<span class="hx-sub">HydroPredict · IrrigSched · Uganda Multi-Location IWR v7.6</span>
</div></div>
<div class="hx-auth">by: Prosper <strong>BYARUHANGA</strong>
&nbsp;·&nbsp; HyPIS App v7.6 &nbsp;·&nbsp; FAO-56 PM · Soil-adjusted MAD · Uganda</div>
...
def depletion_status(dr, raw, taw):
    """
    FIX v7.6 — simplified to canonical FAO-56 rule:
      • dr ≤ 0            → field capacity, no irrigation
      • 0 < dr ≤ RAW×0.5 → soil comfortable, no irrigation
      • RAW×0.5 < dr ≤ RAW→ approaching MAD, monitor
      • dr > RAW          → IRRIGATE — soil is depleted past management
                            threshold regardless of today's rain (Pe was
                            already subtracted when computing dr_new).
      • dr > TAW×0.85     → urgent, near wilting point

    Rain (Pe) is NOT a separate gate here. It has already been
    incorporated into dr_new = max(0, min(TAW, dr_prev − Pe + ETc)).
    Adding a second Pe≥ETc check would double-count rainfall and block
    irrigation even when the root zone is critically depleted.
    """
    if dr <= 0:
    ...
    if dr <= raw:
        return ("🟡 Monitor — nearing MAD", False,
                f"Dr={dr:.1f} mm approaching RAW={raw:.1f} mm — irrigate tomorrow if no rain")
    if dr <= taw * 0.85:
        ...
        if daily is None or daily.empty:
            ...
            if nd > 0:
                ...
                st.success(f"✅ No irrigation needed over next 5 days. "
                           f"({rain_d} rainy day(s) + adequate soil moisture)")
...
st.markdown("---")
st.caption(
    f"HyPIS Ug v7.6 · {SITE_NAME} ({LAT}°, {LON}°, {ELEV} m) · "
