...
def soil_status_label(dr, raw, taw, pe, etc):
    """
    Classify soil moisture status for a given day.
    Returns (status_label, should_irrigate).
    RULE: Only irrigate when Dr > RAW AND Pe < ETc (rain doesn't cover demand).
    """
    if pe >= etc:
        return "🌧️ Rain Covered — No irrigation", False
    if dr <= 0:
        return "🟢 Field Capacity", False
    if dr <= raw * 0.5:
        return "✅ Adequate moisture", False
    if dr <= raw:
        return "🟡 Monitor — nearing threshold", False
    if dr <= taw:
        return "⚠️ Irrigate — below RAW", True
    return "🔴 Wilting Risk!", True
...
with tab1:
    ...
    st.caption(
        ...
        f"Rain-aware: no IWR when Pe ≥ ETc"
    )
    ...
    if st.button("🧮 Calculate Today's IWR + Past 5 Days Context",
        ...
        if hist_ctx is not None and not hist_ctx.empty:
            ...
            styled_past = past_disp.style.applymap(
                _style_status, subset=["Soil Status"])
        ...
        r1.metric("ET₀ FAO-56 PM",  f"{res['et0_fao']:.3f} mm/d")
        r2.metric("ET₀ Hargreaves", f"{res['et0_h']:.3f} mm/d")
        r3.metric("ETc",            f"{res['etc']:.3f} mm/d",  f"Kc={kc1:.3f}")
        ...
        if res["irrigate"]:
            ...
            st.success(
                ...
                f"{'Rain (Pe = ' + str(res['pe']) + ' mm) covers crop demand (ETc = ' + str(res['etc']) + ' mm/d).' if res['pe'] >= res['etc'] else 'Soil still has enough moisture — monitor tomorrow.'}"
            )
        ...
        today_row = pd.DataFrame([{
            ...
            "Rain (mm)": prec_in,
            "ET₀ PM (mm/d)": res["et0_fao"],
            "ET₀ H (mm/d)": res["et0_h"],
            "Kc": kc1,
            "ETc (mm/d)": res["etc"],
            "Pe (mm)": res["pe"],
            "Dr Deficit (mm)": res["dr_today"],
            "RAW threshold (mm)": res["raw"],
            "TAW (mm)": res["taw"],
            "SM % FC (before)": res["sm_pct_now"],
            "NIR (mm)": res["nir"],
        ...
        if hist_ctx is not None and not hist_ctx.empty:
            combined = pd.concat([
                past_disp.rename(columns={
                    "IWR gross (mm)": "IWR gross (mm)",
                    "Deficit Dr (mm)": "Dr Deficit (mm)",
                }),
                today_row.rename(columns={
                    "Rain (mm)": "Rain (mm)",
                }).set_index("Date")[[
                    c for c in past_disp.columns if c in today_row.columns
                ]].rename(columns={"Soil Status": "Soil Status"})
            ], sort=False)
            dl_data = combined.fillna("").to_csv().encode()
        else:
            dl_data = today_row.to_csv(index=False).encode()
...
with tab2:
    ...
    st.caption(
        ...
        f"Rain-aware (IWR=0 when Pe ≥ ETc) · "
...
with tab3:
    ...
    st.caption(
        ...
        f"IWR = 0 on days Pe ≥ ETc · IWR = NIR ÷ Ef ({Ef*100:.0f}%, {irrig_sys})"
    )
