"""
HyPIS Ug  v7.5  — Bug-Fixed
═══════════════════════════════════════════════════════════════════════════════
FIXES in v7.5 (over v7.4):
  ✔ Removed manual rain input entirely — rainfall is now always sourced
    directly from the Open-Meteo forecast (prec_in = lp). No number_input,
    no stale session-state value, no mismatch warning across districts.
  ✔ Replaced rain input widget with read-only st.info() display so it is
    clear the value comes from the forecast, not user entry.
  ✔ Rain mismatch warning block removed completely.
...
with tab1:
    ...
    with col2:
        ...
        # v7.5: rain always comes from the Open-Meteo forecast — no manual input.
        # prec_in is set directly from lp so all downstream code is unchanged.
        prec_in = lp
        st.info(f"🌧️ **Rainfall (from Open-Meteo forecast): {prec_in:.1f} mm**")
