"""
HyPIS Ug  v7.2  — Bug-Fixed + Enhanced
═══════════════════════════════════════════════════════════════════════════════
FIXES in v7.2:
  ✔ CRITICAL: Styler.applymap → Styler.map  (pandas 3.0 removed applymap)
  ✔ DataFrame.applymap → DataFrame.map      (pandas 3.0 removed applymap)
  ✔ Added pandas-version-safe styler helper (forward-compatible)
  ✔ estimate_sm now cached with @st.cache_data to prevent repeated API calls
  ✔ Fixed rh / rh_mean column aliasing in run_water_balance
  ✔ Fixed NoneType crashes when weather fetch returns None mid-calculation
  ✔ Fixed Vol_L dtype (int → float) casting error on large areas
  ✔ Added explicit dtype safety on numeric columns before plotting

ENHANCEMENTS in v7.2:
  ✔ Water Balance Summary card (TAW / RAW / Dr / SM% at a glance)
  ✔ ML model status shown in sidebar with feature list
  ✔ Irrigation calendar heatmap on Tab 1
  ✔ "Quick-Copy" advisory text block for farmer SMS / WhatsApp
  ✔ Soil moisture gauge chart (visual)
  ✔ Combined Excel download (past 5 days + today) alongside CSV

CORE LOGIC (unchanged from v7.1 — FAO-56 compliant):
  ✔ Irrigation trigger: Dr ≥ RAW  AND  Pe < ETc
  ✔ Pe ≥ ETc  →  NIR = 0, IWR = 0
  ✔ NIR shown every day (max(0, ETc–Pe))
  ✔ Soil-texture MAD adjustment (FAO-56 ±5–10%)
  ✔ Past 5 days context table
  ✔ Threshold warnings (🟢🟡⚠️🔴)

Author: Prosper BYARUHANGA · HyPIS App v7.2 · FAO-56 PM
═══════════════════════════════════════════════════════════════════════════════
"""
...
.sms-box{background:#f3e5f5;border:1.5px solid #9c27b0;border-radius:10px;padding:10px 16px;font-size:.87rem;margin:8px 0;font-family:monospace;}
...
<span class="hx-sub">HydroPredict · IrrigSched · Uganda Multi-Location IWR v7.2</span>
</div></div>
<div class="hx-auth">by: Prosper <strong>BYARUHANGA</strong>
&nbsp;·&nbsp; HyPIS App v7.2 &nbsp;·&nbsp; FAO-56 PM · Soil-adjusted MAD · Uganda</div>
...
# ML model status in sidebar
st.sidebar.markdown("---\n### 🤖 ML Model")
if ML_OK:
    st.sidebar.success(ML_STATUS)
    st.sidebar.caption(f"Features: {', '.join(ML_FEATURES)}")
else:
    st.sidebar.warning(ML_STATUS or "No ML model")
...
with tab1:
    ...
    st.caption(
        ...
        f"FAO-56 PM v7.2 · Irrigation trigger: Dr ≥ RAW AND Pe < ETc · "
    ...
    if st.button("🧮 Calculate Today's IWR + Past 5 Days Water History",
        ...
        if hist_ctx is not None and not hist_ctx.empty:
            ...
            # ── FIX: use _styler_map wrapper instead of .applymap ────────────
        ...
        if irrigate:
            ...
            # Quick-copy SMS/WhatsApp advisory
            sms_text = (
                f"[HyPIS Ug v7.2 — {datetime.today().strftime('%d %b %Y')}]\n"
                f"Site: {SITE_NAME} | Crop: {cr1} | Stage: {STAGE_LABELS[stage1]}\n"
                f">>> IRRIGATE TODAY <<<\n"
                f"IWR = {iwr1:.2f} mm | Vol = {vol1['vol_m3']:.1f} m³ ({vol1['vol_L']:,.0f} L)\n"
                f"Pump time: {mins1} min @ {pump_flow} m³/hr | Area: {area_ha} ha\n"
                f"ETc={etc1:.2f} mm · Pe={pe1:.2f} mm · Dr={dr_today:.2f} mm (RAW={raw1:.1f} mm)\n"
                f"SM now: {sm_now}% FC → {sm_aft}% FC after irrigation"
            )
        else:
            st.markdown(
        ...
        # Quick-copy advisory
        st.markdown("#### 📱 Quick Advisory (SMS / WhatsApp)")
        st.markdown(f'<div class="sms-box">{sms_text.replace(chr(10),"<br>")}</div>',
                    unsafe_allow_html=True)
        st.download_button("📋 Copy Advisory Text", sms_text,
                           file_name=f"HyPIS_advisory_{date.today()}.txt",
                           mime="text/plain", key="dl_sms")
        ...
        with dl_col1:
            st.download_button("⬇️ Download CSV", dl_csv,
                               f"{_fn}.csv", "text/csv", key="dl_today_csv")
        with dl_col2:
            st.download_button("⬇️ Download Excel (.xlsx)", dl_xlsx,
                               f"{_fn}.xlsx",
...
with tab2:
    ...
    if st.button("📥 Get 5-Day Forecast", type="primary",
        ...
        if daily is None or daily.empty:
            ...
            fc_dl1, fc_dl2 = st.columns(2)
            _fn_fc = f"HyPIS_forecast_{SITE_NAME.replace(' ','_')}_{date.today()}"
            with fc_dl1:
                st.download_button("⬇️ Download CSV", dl_fc_csv,
                                   f"{_fn_fc}.csv","text/csv", key="dl_fc_csv")
            with fc_dl2:
                st.download_button("⬇️ Download Excel", dl_fc_xlsx,
                                   f"{_fn_fc}.xlsx",
                                   "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                   key="dl_fc_xlsx")
...
with tab3:
    ...
    if st.button("📥 Retrieve Historical Data", type="primary",
        ...
        if hist is None or hist.empty:
            ...
            with h_dl1:
                st.download_button("⬇️ Download CSV", dl_h_csv,
                                   f"{_fn_h}.csv","text/csv",key="dl_hist_csv")
            with h_dl2:
                st.download_button("⬇️ Download Excel", dl_h_xlsx,
                                   f"{_fn_h}.xlsx",
                                   "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                   key="dl_hist_xlsx")
...
st.markdown("---")
st.caption(
    f"HyPIS Ug v7.2 · {SITE_NAME} ({LAT}°, {LON}°, {ELEV} m) · "
