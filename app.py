"""
HyPIS Ug  v7.4  — Bug-Fixed
═══════════════════════════════════════════════════════════════════════════════
FIXES in v7.4 (over v7.3):
  ✔ CRITICAL: Rain display mismatch — banner showed forecast 0.0 mm while
    table showed user-entered rain (e.g. 7 mm). Now:
    - Banner clearly labelled "📡 Forecast (Open-Meteo)"
    - prec_in number input labelled "Rain today — actual observed (mm)"
    - Warning shown when user-entered rain differs from forecast
    - Both values always visible in weather cards (Forecast vs Actual)
  ✔ CRITICAL: Wrong caption trigger text — "rain = 0" replaced by correct
    "Pe < ETc" everywhere. "any Pe > 0 always pauses irrigation" removed
    (only Pe ≥ ETc pauses irrigation per FAO-56).
  ✔ estimate_sm now uses ETc = Kc × ET₀ (default mid-season Kc 1.0) instead
    of raw ET₀, producing more realistic starting SM estimates.
  ✔ SM% rounding changed from int() truncation to round(...,1) throughout,
    eliminating systematic floor bias.
  ✔ Forecast filter now timezone-aware (Africa/Nairobi) so today's row is
    never dropped due to UTC/EAT mismatch.
  ✔ Dead ML model sidebar section removed to avoid user confusion
    (model file missing; model never applied to decisions).
  ✔ Footer caption corrected: trigger wording unified with actual logic.

FIXES in v7.3 (retained):
  ✔ CRITICAL: openpyxl not available on Python 3.14 — Excel downloads now
    gracefully fall back to CSV-only; no crash, no error shown to user
  ✔ df_to_excel_bytes wrapped in try/except; returns None on failure
  ✔ All .xlsx download buttons are conditional on OPENPYXL_OK flag
  ✔ Removed "📱 Quick Advisory (SMS / WhatsApp)" section entirely
  ✔ sms_text variable kept internally for .txt download only
  ✔ Auto-install of openpyxl retained but no longer relied upon
  ✔ All other openpyxl-related crashes in Tab 2 & Tab 3 fixed

FIXES in v7.2 (retained):
  ✔ Styler.applymap → Styler.map  (pandas 3.0)
  ✔ DataFrame.applymap → DataFrame.map
  ✔ estimate_sm cached with @st.cache_data
  ✔ rh / rh_mean column aliasing in run_water_balance
  ✔ NoneType crashes when weather fetch returns None
  ✔ Vol_L dtype (int → float) casting error on large areas
  ✔ Explicit dtype safety on numeric columns before plotting

Author: Prosper BYARUHANGA · HyPIS App v7.4 · FAO-56 PM
═══════════════════════════════════════════════════════════════════════════════
"""
...
# ── pandas version-safe styler ────────────────────────────────────────────────
_PD_VER = tuple(int(x) for x in pd.__version__.split(".")[:2])

def _styler_map(styler, func, subset=None):
    """Forward-compatible wrapper: uses Styler.map (pandas ≥2.0) or applymap."""
    if _PD_VER >= (2, 0):
...
# ── openpyxl availability flag (Python 3.14 may not support it) ──────────────
try:
...
# ══════════════════════════════════════════════════════════════════════════════
# UGANDA LOCATIONS  (lat, lon, elev_m)
# ══════════════════════════════════════════════════════════════════════════════
LOCATIONS = {
...
# ══════════════════════════════════════════════════════════════════════════════
# SOIL DATABASE — Uganda districts (FAO / HWSD v2)
# ══════════════════════════════════════════════════════════════════════════════
DISTRICT_SOIL = {
...
# ══════════════════════════════════════════════════════════════════════════════
# FAO-56 SOIL-TEXTURE MAD ADJUSTMENT  (Table 22 footnote: ±5–10%)
# ══════════════════════════════════════════════════════════════════════════════
TEXTURE_MAD_ADJ = {
    "Sand":            +0.10,
    "Loamy Sand":      +0.08,
...
# ══════════════════════════════════════════════════════════════════════════════
# CROP PARAMETERS  (FAO-56 Table 22)
# ══════════════════════════════════════════════════════════════════════════════
crop_params = {
...
<span class="hx-sub">HydroPredict · IrrigSched · Uganda Multi-Location IWR v7.4</span>
</div></div>
<div class="hx-auth">by: Prosper <strong>BYARUHANGA</strong>
&nbsp;·&nbsp; HyPIS App v7.4 &nbsp;·&nbsp; FAO-56 PM · Soil-adjusted MAD · Uganda</div>
...
# ML model status — only show in sidebar if the file is actually present
# FIX v7.4: removed prominent ML sidebar section; model is loaded but NOT used
# in any decision logic (ML_MODEL.predict() is never called). Showing it as
# a green "✅ loaded" badge was misleading. When the .pkl file is missing
# (the common case) it still showed a confusing warning every load.
if ML_OK:
    st.sidebar.markdown("---")
...
def depletion_status(dr, raw, taw, pe, etc):
    """
    FAO-56 irrigation trigger.
    Irrigate ONLY when Dr ≥ RAW  AND  Pe < ETc.
    """
    rain_ok = (pe >= etc)
    if rain_ok:
        return "🌧️ Rain covers crop demand", False, "Pe ≥ ETc — no irrigation needed today"
    if dr <= 0:
        return "🟢 Field Capacity", False, "Soil at FC — no irrigation"
    if dr <= raw * 0.5:
        return "✅ Adequate moisture", False, "Soil water adequate — no irrigation"
    if dr <= raw:
        return "🟡 Monitor — nearing MAD", False, f"Dr approaching RAW ({raw:.1f} mm) — irrigate tomorrow if no rain"
    if dr <= taw * 0.85:
        return "⚠️ Below MAD — irrigate", True,  f"Dr={dr:.1f} mm > RAW={raw:.1f} mm — crop stress starting"
    return "🔴 Near wilting point — URGENT", True, f"Dr={dr:.1f} mm ≈ TAW={taw:.1f} mm — immediate irrigation!"
...
def run_water_balance(daily_df, crop, soil, planting_ts, sm_pct,
    ...
    # Normalise RH columns — ensure rh_max / rh_min exist
    ...
    for _, row in df.iterrows():
        ...
        lbl, irrigate, note = depletion_status(dr_new, raw, taw, pe_r, etc_r)
        ...
        # FIX v7.4: use round() not int() — int() systematically floors, biasing SM% low
...
# ══════════════════════════════════════════════════════════════════════════════
# SOIL MOISTURE ESTIMATION  (ERA5 10-day back-run) — cached
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_data(ttl=3600, show_spinner=False)
def estimate_sm(fc, pwp, zr, lat=None, lon=None, elev=None, default_kc=1.0):
    """
    Estimate current soil moisture by running a 10-day back water-balance
    using ERA5 archive data (Open-Meteo).

    FIX v7.4: Now uses ETc = default_kc × ET₀ (mid-season Kc ≈ 1.0) instead
    of raw ET₀, giving a more realistic depletion estimate.
    """
    ...
    try:
        ...
        taw_ = (fc-pwp)*zr*1000.
        theta = pwp+0.70*(fc-pwp); dr_=max(0.,(fc-theta)*zr*1000.)
        for i in range(len(dates)):
            ...
            # FIX: multiply by default_kc (≈1.0) to get ETc, not just ET₀
            etc_i = et0_i * default_kc
            pe_i=eff_rain(prec); dr_=max(0.,min(taw_,dr_-pe_i+etc_i))
        return int(max(0,min(100,(1-dr_/taw_)*100))) if taw_>0 else 70
    except Exception:
        return 60

# ══════════════════════════════════════════════════════════════════════════════
# EXCEL EXPORT HELPER  — safe: returns None if openpyxl unavailable
# ══════════════════════════════════════════════════════════════════════════════
def df_to_excel_bytes(df_dict: dict):
    """
    Convert a dict of {sheet_name: DataFrame} to Excel bytes.
    Returns None (not bytes) when openpyxl is not available, so callers
    can skip the download button gracefully instead of crashing.
    """
    if not OPENPYXL_OK:
        return None
    try:
...
def _show_download_buttons(dl_csv, dl_xlsx, fn_base, csv_key, xlsx_key):
    """Helper that renders CSV button always, Excel button only when available."""
...
def sm_gauge(sm_pct, raw_pct_of_taw, title="Soil Moisture"):
    """Plotly gauge showing soil moisture % of FC with RAW threshold."""
...
def get_forecast(_k, lat, lon, elev):
    try:
        ...
        # FIX v7.4: use timezone-aware today so Uganda rows (UTC+3) are never dropped
...
with tab1:
    ...
    # FIX v7.4: corrected trigger description — old text said "rain = 0" which is WRONG.
    # Correct FAO-56 rule: irrigate when Dr ≥ RAW AND Pe < ETc (not just when rain = 0).
    # Also removed "Rain event (any Pe > 0) always pauses irrigation" — only Pe ≥ ETc pauses.
    st.caption(
        f"📡 {wx['source'] if wx else 'Weather unavailable'} · "
        f"FAO-56 PM v7.4 · Trigger: Dr ≥ RAW **AND** Pe < ETc · "
        f"Rain pauses irrigation only when Pe ≥ ETc (rain covers full crop demand) · "
        f"All values per 24-hour day"
    )

    if wx:
        # FIX v7.4: Label clearly as FORECAST so user knows lp is predicted, not observed.
        # The number input below (prec_in) is where actual observed rain is entered.
        st.success(
            f"✅ **{wx['description']}** · 📡 Forecast Rain: **{lp} mm** *(Open-Meteo prediction — "
            f"update 'Rain today' input below with actual observed rain if it differs)* · "
        ...
        # FIX v7.4: label as "Forecast" — this is NOT observed rain, it is Open-Meteo's
        # day-total forecast. User must enter actual observed rain in the input below.
    ...
    with col1:
        ...
        st.markdown(
            ...
            f'Crop MAD (p) = <b>{mad_crop:.2f}</b>'
            f'{"" if mad_delta==0 else f" → soil-adjusted to <b>{mad_adj:.2f}</b> "}'
            f'<small>({"+0.05 sandy" if mad_delta>0 else "−0.05 clay" if mad_delta<0 else "Loam reference"})</small><br>'
            f'RAW (trigger) = <b>{_raw_p:.1f} mm</b> &nbsp;·&nbsp; '
            f'Irrigate when Dr &gt; <b>{_raw_p:.1f} mm</b> AND rain &lt; ETc<br>'
    ...
    with col2:
        ...
        # FIX v7.4 (CRITICAL): Renamed label to make clear this is ACTUAL rain,
        # not the forecast shown in the banner above. Default still set to forecast (lp)
        # but user MUST update this if actual observed rain differs.
        prec_in = st.number_input(
            "Rain today — actual observed (mm)",
            value=float(lp), min_value=0., key="p1",
            help=(
                f"📡 Open-Meteo forecast for today: {lp:.1f} mm. "
                "If it actually rained a different amount, enter the real observed value here. "
                "This is the value used in all calculations below."
            )
        )

        # FIX v7.4: Warn user when their input differs from the forecast
        # (this is exactly the mismatch that caused the banner vs table discrepancy)
        if abs(prec_in - lp) > 0.5:
            st.warning(
                f"⚠️ **Rain mismatch**: forecast = **{lp:.1f} mm** (banner above) but "
                f"you entered **{prec_in:.1f} mm** below. "
                f"All water-balance calculations use your entered value ({prec_in:.1f} mm). "
                f"The banner always shows the Open-Meteo forecast and will not update."
            )
        ...
        # FIX v7.4: Add weather input source note so the user understands
        # prec_in is where ACTUAL observed rain goes, separate from the forecast banner.
        st.markdown(
    ...
    if st.button("🧮 Calculate Today's IWR + Past 5 Days Water History",
        ...
        status_lbl, irrigate, note_today = depletion_status(dr_today, raw1, taw1, pe1, etc1)
        ...
        # FIX v7.4: use round() not int() to avoid systematic floor bias
        ...
        with gcol2:
            ...
            if "🔴" in status_lbl:
                ...
                st.markdown(
                    f'<div class="nir-box">🟡 <b>Monitor</b> — Dr approaching MAD threshold. '
                    f'Dr = {dr_today:.1f} mm (RAW = {raw1:.1f} mm). '
                    f'Irrigate tomorrow if no rain.</div>', unsafe_allow_html=True)
            else:
                st.markdown(
            ...
            st.markdown(
                f'<div class="nir-box">📐 <b>Daily NIR (crop deficit) = {nir1_day:.2f} mm</b> '
        ...
        st.markdown('<div class="past-hdr">📅 PAST 5 DAYS — Soil & Crop Water History</div>',
                    unsafe_allow_html=True)
        st.caption(
            "This table shows how soil moisture and daily crop water need evolved "
            "BEFORE today — so you can see the full picture behind today's decision.")
        ...
        if hist_ctx is not None and not hist_ctx.empty:
            ...
            for dt, row in past_r.iterrows():
                rows_past.append({
                    ...
                    "Dr Deficit (mm)":    round(float(row["Dr_mm"]),2),
                    "SM (%FC)":           int(row["SM_pct"]),
                    "NIR (mm)":           round(float(row["NIR"]),2),
                    "IWR gross (mm)":     round(float(row["IWR"]),2),
                    "Vol needed (m³)":    float(row["Vol_m3"]),
                    "Soil Status":        row["Status"],
            ...
            st.markdown(
                ...
                f'<small>NIR = ETc – Pe (daily deficit, shown always) &nbsp;|&nbsp; '
                f'IWR = 0 unless Dr &gt; RAW and Pe &lt; ETc</small></div>',
                unsafe_allow_html=True)

            styled = _styler_map(past_df.style, _style_row, subset=["Soil Status"])
            ...
            fig_ctx.add_bar(
                ...
                name="Rain (mm)",
                marker_color="#1a5fc8", opacity=0.4, yaxis="y2")
            fig_ctx.add_bar(
                x=past_r.index.strftime("%a %d"),
            ...
            st.info("ℹ️ ERA5 past data unavailable (archive may lag 3–5 days). "
                    "Showing today's result only.")

        # ════════════════════════════════════════════════════════════════════
        # TODAY'S RESULT
        # ════════════════════════════════════════════════════════════════════
        st.markdown('<div class="today-hdr">💧 TODAY\'S IRRIGATION DECISION (24-hour period)</div>',
                    unsafe_allow_html=True)
        ...
        r1.metric("ET₀ PM (mm/d)",   f"{et0_fao:.3f}")
        r2.metric("ET₀ H (mm/d)",    f"{et0_h:.3f}")
        r3.metric("ETc (mm/d)",      f"{etc1:.3f}", f"Kc={kc1:.3f}")
        r4.metric("Pe (mm)",         f"{pe1:.2f}")
        r5.metric("NIR (mm/d)",      f"{nir1_day:.2f}", "ETc − Pe")
        r6.metric("Dr Deficit (mm)", f"{dr_today:.2f}", f"RAW={raw1:.1f}")
        r7.metric("SM % FC",         f"{sm_now}%", f"→{sm_aft}% after")

        st.markdown("---")

        if irrigate:
            st.markdown(
                f'<div class="refill-box">🔧 <b>Irrigation to refill root zone to FC:</b> '
            ...
            st.markdown(
                f'<div class="no-irr-box">✅ <b>No irrigation needed today (24-hour period)</b><br>'
                f'{note_today}<br>'
                f'Volume to apply = <b>0 m³</b> &nbsp;·&nbsp; Pump time = <b>0 min</b></div>',
                unsafe_allow_html=True)
            if pe1 >= etc1:
                st.success(
                    f"🌧️ **Rain covers crop demand today.**  \n"
                    f"Pe = {pe1:.2f} mm ≥ ETc = {etc1:.2f} mm/d → NIR = 0, IWR = 0.  \n"
                    f"Soil Dr = {dr_today:.2f} mm · SM = {sm_now}% of FC.")
            else:
                st.success(
                    f"✅ **No irrigation needed — soil moisture is adequate.**  \n"
                    f"Dr = {dr_today:.2f} mm ≤ RAW = {raw1:.1f} mm.  \n"
                    f"Crop daily deficit (NIR) = {nir1_day:.2f} mm/d — "
                    f"soil buffer covers this without irrigation today.")

        # ── TODAY'S SUMMARY TABLE ────────────────────────────────────────────
        st.markdown("#### 📋 Today's Daily Summary")
        # FIX v7.4: Show both forecast and actual rain so discrepancy is always visible
        today_tbl = pd.DataFrame([{
            "Date":                    datetime.today().strftime("%Y-%m-%d (%a)"),
            "Rain forecast (mm)":      lp,           # from Open-Meteo (banner value)
            "Rain actual/used (mm)":   prec_in,      # what user entered / used in calc
            "ETc (mm/d)":              etc1,
            "Pe (mm)":                 pe1,
            "Dr Deficit (mm)":         round(dr_today, 2),
            "SM (%FC)":                sm_now,
            "NIR (mm)":                nir1_day,
            "IWR gross (mm)":          iwr1,
            "Vol needed (m³)":         vol1["vol_m3"],
            "Vol (L)":                 vol1["vol_L"],
            "Pump time (min)":         mins1,
            "ET₀ PM (mm/d)":           et0_fao,
            "ET₀ H (mm/d)":            et0_h,
        ...
        st.dataframe(today_tbl, use_container_width=True)
        ...
        if past_r is not None:
            ...
            for dt, row in past_r.iterrows():
                combined_rows.append({
                    ...
                    "NIR (mm)": round(float(row["NIR"]),2),
                    "IWR gross (mm)": round(float(row["IWR"]),2),
                    "Vol (m³)": float(row["Vol_m3"]), "Status": row["Status"],
                })
            combined_rows.append({
                "Date": datetime.today().strftime("%Y-%m-%d"), "Period": "TODAY",
                "Rain (mm)": prec_in, "ETc (mm/d)": etc1, "Pe (mm)": pe1,
                "Dr Deficit (mm)": round(dr_today,2), "SM (%FC)": sm_now,
                "NIR (mm)": nir1_day, "IWR gross (mm)": iwr1,
                "Vol (m³)": vol1["vol_m3"], "Status": status_lbl,
            })
            combined_df = pd.DataFrame(combined_rows).set_index("Date")
            dl_csv  = combined_df.to_csv().encode()
            dl_xlsx = df_to_excel_bytes({
                "Past5Days+Today": combined_df,
                "TodaySummary": today_tbl,
            })
        else:
...
with tab2:
    ...
    st.caption(
        f"FAO-56 PM · Trigger: Dr ≥ RAW AND Pe < ETc · IWR = 0 on rainy days (Pe ≥ ETc) · "
        f"NIR shown daily · Ef={Ef*100:.0f}% ({irrig_sys})"
    )
    ...
    with fc_c2:
        ...
        sm_pct2 = st.slider("Starting SM (% of FC)", 0, 100,
                             estimate_sm(ACTIVE_FC,ACTIVE_PWP,cp2["zr"],LAT,LON,ELEV), key="sm2")
    ...
    if st.button("📥 Get 5-Day Forecast", type="primary",
        ...
        if daily is None or daily.empty:
            ...
            st.info(
                f"📐 TAW = **{taw2:.1f} mm** · RAW (trigger) = **{raw2:.1f} mm** "
                f"(MAD {cp2['mad']:.2f}→{mad2:.2f})  \n"
                f"Irrigation fires only when Dr > {raw2:.1f} mm AND Pe < ETc on that day.")

            if nd > 0:
                st.warning(
                    f"🗓️ **{nd} irrigation event(s)** · {rain_d} rainy day(s) → IWR=0 on rain days  \n"
                    f"Total IWR = **{daily_r['IWR'].sum():.1f} mm** · "
                    f"Vol = **{daily_r['Vol_m3'].sum():.1f} m³**")
            else:
                st.success(f"✅ No irrigation needed over next 5 days. "
                           f"({rain_d} rainy day(s) + sufficient soil moisture)")
            ...
            tb2 = daily_r[[c for c in cols_fc if c in daily_r.columns]].round(3).copy()
            tb2.rename(columns={
                "tmax":"Tmax°C","tmin":"Tmin°C","rh_mean":"RH%",
                "precipitation":"Rain mm","Pe":"Pe mm",
                "ET0":"ET₀ mm/d","kc":"Kc","ETc":"ETc mm/d",
                "Dr_mm":"Dr Deficit mm","SM_pct":"SM %FC",
            ...
            fig2d.update_layout(
                title=f"5-Day Forecast: Dr, NIR, IWR — {SITE_NAME}",
...
with tab3:
    ...
    st.caption(
        f"ERA5 Archive · FAO-56 PM · Dr-threshold trigger · "
        f"IWR=0 on days Pe≥ETc · NIR shown every day · "
    ...
    with hc2:
        ...
        sm3    = st.slider("Starting SM (% of FC)",0,100,
                           estimate_sm(ACTIVE_FC,ACTIVE_PWP,cp3["zr"],LAT,LON,ELEV),key="sm3")
        st.markdown(f'<div class="soil-panel">🌍 <b>{ACTIVE_TXT}</b> · '
                    f'FC={ACTIVE_FC*100:.0f}% · PWP={ACTIVE_PWP*100:.0f}%</div>',
                    unsafe_allow_html=True)
    ...
    if st.button("📥 Retrieve Historical Data", type="primary",
        ...
        if hist is None or hist.empty:
            ...
            st.info(
                ...
                f"Irrigation fires only when Dr > {raw3:.1f} mm AND Pe < ETc.")
            ...
            fig3d.add_scatter(x=hist_r.index,y=hist_r["Dr_mm"].astype(float),mode="lines",
                              name="Dr Deficit mm",line=dict(color="#e6550d",width=1.5))
...
st.markdown("---")
st.caption(
    # FIX v7.4: corrected trigger wording — was "rain = 0" (wrong), now "Pe < ETc" (correct).
    # Also removed "IWR=0 on rain days" shorthand which was misleading
    # (IWR can still be >0 on a light rain day when Pe < ETc and Dr > RAW).
    f"HyPIS Ug v7.4 · {SITE_NAME} ({LAT}°, {LON}°, {ELEV} m) · "
    f"ERA5+ICON+GFS (Open-Meteo) · FAO-56 Penman-Monteith · "
    f"Soil-adjusted MAD (FAO-56 ±5–10% texture rule) · "
    f"Trigger: Dr ≥ RAW AND Pe < ETc · IWR=0 when Pe ≥ ETc (rain covers full crop demand) · "
