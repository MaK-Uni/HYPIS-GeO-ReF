    ...
    if dr <= raw:
        ...
        if ef is not None and ef > 0:
            ...
            note = (f"Dr={dr:.1f} mm approaching RAW={raw:.1f} mm — "
                    ...
                    f"(= {round(dr, 1)} L/m² net) now to refill; "
...
with tab1:
    ...
    if st.button("🧮 Calculate Today's IWR + Past 5 Days Water History",
        ...
        # irr_to_fc == iwr1 when irrigate=True (nir1=dr_today, iwr1=dr_today/Ef)
        # vol_to_fc == vol1 — reuse existing variables, no redundant calculation needed
        ...
        if irrigate:
            st.markdown(
                ...
                f'Full refill gross = <b>{iwr1:.2f} mm</b> = '
                f'{vol1["vol_m3"]:.1f} m³ = {vol1["vol_L"]:,.0f} L</div>',
                unsafe_allow_html=True)
            st.markdown(
