"""
HyPIS Ug  v7.1  — Full Supervisor-Review Implementation
═══════════════════════════════════════════════════════════════════════════════
All changes driven by supervisor's FAO-56 review document:

  CORE LOGIC FIXES
  ✔ Irrigation trigger: Dr ≥ RAW  AND  Pe < ETc  (both conditions required)
  ✔ Pe ≥ ETc  →  NIR = 0, IWR = 0, status = "🌧️ Rain covers crop demand"
  ✔ NIR is shown EVERY day (max(0, ETc–Pe)) so farmer sees daily crop deficit
  ✔ IWR fires only when Dr crosses the MAD/RAW threshold — NOT daily
  ✔ After irrigation: Dr resets to 0 (soil refilled to Field Capacity)
  ✔ Refill-to-FC advisory: if SM extremely low, extra volume flagged
  ✔ Permanent Wilting Point guard: warns if Dr approaches TAW

  MAD / SOIL TEXTURE ADJUSTMENT  (FAO-56 ±5–10% rule)
  ✔ Sandy soils  → p (MAD) increased by +0.05 to +0.10
  ✔ Clay soils   → p (MAD) decreased by –0.05 to –0.10
  ✔ Effective MAD is auto-calculated per crop × soil combination
  ✔ Both crop-based MAD and soil-adjusted MAD displayed to user

  PAST DAYS CONTEXT TABLE  (supervisor's exact requested format)
  ✔ Tab 1 "Today's IWR" shows last 5 days BEFORE today's result:
    Date | Rain | ETc | Pe | Depletion(Dr) | NIR | IWR | Volume | SM%FC | Status
  ✔ Each row has inline plain-English note explaining the day's decision
  ✔ Depletion sparkline vs RAW / TAW threshold lines
  ✔ Today's row appended at bottom so farmer sees full 6-row timeline

  THRESHOLD WARNINGS (FAO-56 Table 22 / Table 19 aligned)
  ✔ Dr ≤ RAW×0.5          → 🟢 Adequate
  ✔ RAW×0.5 < Dr ≤ RAW   → 🟡 Monitor — approaching MAD threshold
  ✔ Dr > RAW              → ⚠️  Below MAD — irrigate today
  ✔ Dr > TAW×0.85         → 🔴  Near wilting point — URGENT

  DAILY 24-HOUR BASIS
  ✔ All values labelled as per-day
  ✔ Volume and pump time shown only when IWR > 0; explicit 0 otherwise

Author: Prosper BYARUHANGA · HyPIS App v7.1 · FAO-56 PM
═══════════════════════════════════════════════════════════════════════════════
"""

import os, sys, time as _time, subprocess, pathlib
import numpy as np
import pandas as pd
import requests
...
# Sandy soils allow crops to deplete MORE before stress (higher p)
# Clay soils reach stress EARLIER (lower p)
# ══════════════════════════════════════════════════════════════════════════════
TEXTURE_MAD_ADJ = {
    "Sand":            +0.10,
    "Loamy Sand":      +0.08,
    ...
    "Loam":            +0.00,   # FAO-56 reference texture
    "Silt Loam":       -0.05,
    "Silt":            -0.05,
    "Clay Loam":       -0.05,
...
def adjust_mad_for_soil(mad_crop, texture):
    """
    FAO-56 footnote (Table 22): p values increase ~0.05–0.10 for coarse soils
    and decrease ~0.05–0.10 for fine/clay soils.
    Returns adjusted MAD clamped to [0.10, 0.90].
    """
    adj  = TEXTURE_MAD_ADJ.get(texture, 0.0)
    # Also handle partial texture name match (e.g. "Loam (default)")
    if adj == 0.0:
...
# ══════════════════════════════════════════════════════════════════════════════
# CROP PARAMETERS  (FAO-56 Table 22 p-values)
# ══════════════════════════════════════════════════════════════════════════════
crop_params = {
    #                  ini    mid    end    zr(m)  mad(p)
...
<span class="hx-sub">HydroPredict · IrrigSched · Uganda Multi-Location IWR v7.1</span>
</div></div>
<div class="hx-auth">by: Prosper <strong>BYARUHANGA</strong>
&nbsp;·&nbsp; HyPIS App v7.1 &nbsp;·&nbsp; FAO-56 PM · Soil-adjusted MAD · Uganda</div>
...
def compute_taw(fc, pwp, zr):
    """TAW (mm) = (FC – PWP) × Zr × 1000  [FAO-56 Eq. 82]"""
    return (fc - pwp) * zr * 1000.

def compute_raw(taw, mad):
    """RAW (mm) = p × TAW  [FAO-56 Eq. 83]  — irrigation trigger threshold"""
    return mad * taw
...
def depletion_status(dr, raw, taw, pe, etc):
    """
    Returns (emoji_label, should_irrigate, note).
    IRRIGATION RULE (FAO-56):
      - Irrigate ONLY when Dr ≥ RAW  AND  Pe < ETc
      - If Pe ≥ ETc → rain covers demand → IWR = 0 regardless of Dr
    """
    rain_ok = (pe >= etc)
    if rain_ok:
...
def run_water_balance(daily_df, crop, soil, planting_ts, sm_pct,
    ...
    """
    FAO-56 daily root-zone water balance.

    TRIGGER: irrigate only when  Dr ≥ RAW  AND  Pe < ETc
    AMOUNT : NIR = Dr (refill to FC); IWR = NIR / Ef
    NIR column: always shows max(0, ETc – Pe) — daily crop water deficit
    IWR column: 0 unless trigger fires

    mad_eff  — pre-computed soil-adjusted MAD (if None, uses crop mad)
    """
    cp   = crop_params[crop]
    zr   = cp["zr"]
    ...
    df["Pe"]  = df.get(prec_col, pd.Series(0., index=df.index)).apply(eff_rain)

    dr_vals=[]; nir_vals=[]; iwr_vals=[]; status_vals=[]; sm_vals=[]; note_vals=[]

    for _, row in df.iterrows():
        pe_r   = row["Pe"];  etc_r  = row["ETc"]
        prec_r = row.get(prec_col, 0.)

        # Daily depletion update (Dr increases with ETc, decreases with Pe)
        dr_new = max(0., min(taw, dr - pe_r + etc_r))

        # NIR = daily crop water deficit (informational every day)
        ...
        sm_now = max(0, min(100, int((1. - dr/taw)*100))) if taw > 0 else 70
    ...
    df["Dr_mm"]   = dr_vals    # root-zone depletion from FC
    df["SM_pct"]  = sm_vals
    df["NIR"]     = nir_vals   # daily crop deficit (ETc-Pe), shown always
    df["IWR"]     = iwr_vals   # actual irrigation (0 unless trigger fires)
...
# ══════════════════════════════════════════════════════════════════════════════
def estimate_sm(fc, pwp, zr, lat=None, lon=None, elev=None):
    ...
    try:
        ...
        for i in range(len(dates)):
            ...
            pe_i=eff_rain(prec); dr_=max(0.,min(taw_,dr_-pe_i+et0_i))
    ...
    try:
        ...
        for i in range(len(d["time"])):
            ...
            rows.append({"date":pd.to_datetime(d["time"][i]),
                ...
                "precipitation":d["precipitation_sum"][i] or 0.,"weather_code":d["weather_code"][i] or 0})
        df=pd.DataFrame(rows).set_index("date")
        return df[df.index>=pd.Timestamp.today().normalize()].head(5)
    except Exception: return None
...
with tab1:
    ...
    st.caption(
        ...
        f"FAO-56 PM v7.1 · Irrigation trigger: Dr ≥ RAW AND Pe < ETc · "
    ...
    with col2:
        ...
        prec_in = st.number_input("Rain today (mm)",      value=float(lp),      min_value=0., key="p1")
    ...
    if st.button("🧮 Calculate Today's IWR + Past 5 Days Water History",
        ...
        # ── Compute effective MAD for this crop × soil ───────────────────────
        ...
        nir1_day = round(max(0., etc1 - pe1), 3)   # daily crop deficit (informational)

        # ── Today's root-zone depletion update ──────────────────────────────
        ...
        if irrigate:
            nir1       = round(dr_today, 3)   # refill to FC
        ...
        sm_now = max(0,min(100,int((1-dr_today/taw1)*100))) if taw1>0 else 70
        sm_aft = max(0,min(100,int((1-dr_after/taw1)*100))) if taw1>0 else 100

        # ── Refill-to-FC advisory (severe deficit) ──────────────────────────
        irr_to_fc = round(dr_today / max(Ef,0.01), 3)   # volume to fully refill
        ...
        st.markdown('<div class="past-hdr">📅 PAST 5 DAYS — Soil & Crop Water History</div>',
                    unsafe_allow_html=True)
        st.caption(
            "This table shows how the soil moisture and daily crop water need evolved "
        ...
        if hist_ctx is not None and not hist_ctx.empty:
            ...
            # ── Build display table matching supervisor's exact format ───────
            rows_past = []
            for dt, row in past_r.iterrows():
                rows_past.append({
                    "Date":               dt.strftime("%Y-%m-%d (%a)"),
                    "Rain (mm)":          round(row.get("precipitation",0.),1),
                    "ETc (mm/d)":         round(row["ETc"],2),
                    "Pe (mm)":            round(row["Pe"],2),
                    "Dr Deficit (mm)":    round(row["Dr_mm"],2),
                    "SM (%FC)":           row["SM_pct"],
                    "NIR (mm)":           round(row["NIR"],2),
                    "IWR gross (mm)":     round(row["IWR"],2),
                    "Vol needed (m³)":    row["Vol_m3"],
                    "Soil Status":        row["Status"],
                    "Decision Note":      row["Note"],
                })
            past_df = pd.DataFrame(rows_past).set_index("Date")

            # Reference thresholds box
            st.markdown(
                f'<div class="mad-panel">'
                f'📐 <b>Reference thresholds — {cr1} / {ACTIVE_TXT}</b><br>'
                f'TAW = <b>{taw1:.1f} mm</b> (total water available) &nbsp;·&nbsp; '
                f'RAW = <b>{raw1:.1f} mm</b> (irrigate when Dr exceeds this) &nbsp;·&nbsp; '
                f'MAD = <b>{mad_eff1:.2f}</b> (soil-adjusted from {cp1["mad"]:.2f})<br>'
                f'<small>NIR = daily crop water deficit (ETc – Pe, shown every day) &nbsp;|&nbsp; '
            ...
            def _style_row(val):
                if "⚠️" in str(val) or "🔴" in str(val):
                    return "background:#ffe0e0;font-weight:bold"
                if "🌧️" in str(val):  return "background:#e0f7fa"
                if "🟢" in str(val) or "✅" in str(val): return "background:#e8f5e9"
                if "🟡" in str(val):  return "background:#fff9c4"
                return ""

            styled = past_df.style.applymap(_style_row, subset=["Soil Status"])
            ...
            fig_ctx.add_scatter(
                ...
                y=past_r["Dr_mm"], mode="lines+markers+text",
                name="Dr Deficit (mm)",
            ...
            fig_ctx.add_bar(
                x=past_r.index.strftime("%a %d"),
                y=past_r["precipitation"], name="Rain (mm)",
                marker_color="#1a5fc8", opacity=0.4, yaxis="y2")
            fig_ctx.add_bar(
                x=past_r.index.strftime("%a %d"),
                y=past_r["NIR"], name="NIR mm/d",
        ...
        # ── TODAY'S SUMMARY TABLE  (matches supervisor's exact column format) ─
        ...
        # ── COMBINED CSV DOWNLOAD (past 5 days + today) ─────────────────────
        if hist_ctx is not None and not hist_ctx.empty:
            combined_rows = []
            for dt, row in past_r.iterrows():
                combined_rows.append({
                    ...
                    "Rain (mm)": round(row.get("precipitation",0.),1),
                    "ETc (mm/d)": round(row["ETc"],2), "Pe (mm)": round(row["Pe"],2),
                    "Dr Deficit (mm)": round(row["Dr_mm"],2), "SM (%FC)": row["SM_pct"],
                    "NIR (mm)": round(row["NIR"],2),
                    "IWR gross (mm)": round(row["IWR"],2),
                    "Vol (m³)": row["Vol_m3"], "Status": row["Status"],
                })
            combined_rows.append({
            ...
            dl_csv = pd.DataFrame(combined_rows).set_index("Date").to_csv().encode()
        else:
        ...
        st.download_button(
            ...
            dl_csv,
            f"HyPIS_{SITE_NAME.replace(' ','_')}_{datetime.today().strftime('%Y%m%d')}.csv",
...
with tab2:
    ...
    with fc_c2:
        ...
        sm_pct2 = st.slider("Starting SM (% of FC)", 0, 100,
                             estimate_sm(ACTIVE_FC,ACTIVE_PWP,cp2["zr"]), key="sm2")
    ...
    if st.button("📥 Get 5-Day Forecast", type="primary",
        ...
        if daily is None or daily.empty:
            ...
            rain_d  = (daily_r["precipitation"]>1).sum()
            ...
            # Depletion chart
            fig2d = go.Figure()
            fig2d.add_scatter(x=daily_r.index.strftime("%a %d"), y=daily_r["Dr_mm"],
                              mode="lines+markers+text", name="Dr Deficit (mm)",
                              text=daily_r["Dr_mm"].round(1).astype(str),
                              textposition="top center",
                              line=dict(color="#e6550d",width=2))
            fig2d.add_bar(x=daily_r.index.strftime("%a %d"), y=daily_r["precipitation"],
                          name="Rain (mm)", marker_color="#1a5fc8",opacity=0.4,yaxis="y2")
            fig2d.add_bar(x=daily_r.index.strftime("%a %d"), y=daily_r["IWR"],
                          name="IWR gross (mm)", marker_color="#17a2b8",opacity=0.7,yaxis="y2")
            fig2d.add_bar(x=daily_r.index.strftime("%a %d"), y=daily_r["NIR"],
            ...
            st.download_button("⬇️ Download Forecast CSV",
                               tb2.to_csv().encode(),
                               f"HyPIS_forecast_{SITE_NAME.replace(' ','_')}_{date.today()}.csv",
                               "text/csv", key="dl_fc")
...
with tab3:
    ...
    if st.button("📥 Retrieve Historical Data", type="primary",
        ...
        if hist is None or hist.empty:
            ...
            # Table — supervisor format: Date, Rain, ETc, Pe, Dr, SM%FC, NIR, IWR, Vol, Status
            ...
            # Depletion chart
            fig3d = go.Figure()
            fig3d.add_scatter(x=hist_r.index,y=hist_r["Dr_mm"],mode="lines",
                              name="Dr Deficit mm",line=dict(color="#e6550d",width=1.5))
            fig3d.add_bar(x=hist_r.index,y=hist_r["precipitation"],name="Rain mm",
                          marker_color="#1a5fc8",opacity=0.35,yaxis="y2")
            fig3d.add_bar(x=hist_r.index,y=hist_r["IWR"],name="IWR gross mm",
                          marker_color="#17a2b8",opacity=0.7,yaxis="y2")
            fig3d.add_bar(x=hist_r.index,y=hist_r["NIR"],name="NIR mm/d",
                          marker_color="#0b6b1b",opacity=0.4,yaxis="y2")
            ...
            # ET₀ chart
            fig3e = go.Figure()
            fig3e.add_scatter(x=hist_r.index,y=hist_r["ET0"],name="ET₀ PM",
                              mode="lines",line=dict(color="#1a5fc8",width=1.5))
            fig3e.add_scatter(x=hist_r.index,y=hist_r["ET0_H"],name="ET₀ Hargreaves",
                              mode="lines",line=dict(color="#b81c1c",width=1,dash="dot"))
            fig3e.add_scatter(x=hist_r.index,y=hist_r["ETc"],name="ETc",
            ...
            # Volume on irrigated days only
            irrig_d = hist_r[hist_r["IWR"]>0]
            if not irrig_d.empty:
                fig3v = go.Figure()
                fig3v.add_bar(x=irrig_d.index,y=irrig_d["Vol_m3"],
            ...
            st.download_button("⬇️ Download Historical CSV",
                               ht.to_csv().encode(),
                               f"HyPIS_hist_{SITE_NAME.replace(' ','_')}_{h_start}_{h_end}.csv",
...
st.markdown("---")
st.caption(
    f"HyPIS Ug v7.1 · {SITE_NAME} ({LAT}°, {LON}°, {ELEV} m) · "
    f"ERA5+ICON+GFS (Open-Meteo) · FAO-56 Penman-Monteith · "
    f"Soil-adjusted MAD (FAO-56 ±5–10% texture rule) · "
    f"Trigger: Dr≥RAW AND Pe<ETc · IWR=0 on rain days · "
    f"HWSD v2 Soil · Byaruhanga Prosper"
)
